...
export const URLToFilters = (url: string): ViewParamURLValues => {
  ...
  searchParams.forEach((value, key) => {
    const parsedValue = value.includes('"') ? value.replaceAll('"', "") : value;
...
export const getDataLayersFromBounds = (bounds: {
  ...
  return [
    {
      label: "0",
      color: ChoroplethMapColors.land,
    },
    {
      label:
        bounds.level1.lower.text === bounds.level1.upper.text
          ? bounds.level1.upper.text
          : `${bounds.level1.lower.text}-${bounds.level1.upper.text}`,
      color: ChoroplethMapColors.level1,
    },
    {
      label:
        bounds.level2.lower.text === bounds.level2.upper.text
          ? bounds.level2.upper.text
          : `${bounds.level2.lower.text}-${bounds.level2.upper.text}`,
      color: ChoroplethMapColors.level2,
    },
    {
      label:
        bounds.level3.lower.text === bounds.level3.upper.text
          ? bounds.level3.upper.text
          : `${bounds.level3.lower.text}-${bounds.level3.upper.text}`,
      color: ChoroplethMapColors.level3,
    },
    {
      label:
        bounds.level4.lower.text === bounds.level4.upper.text
          ? bounds.level4.upper.text
          : `${bounds.level4.lower.text}-${bounds.level4.upper.text}`,
      color: ChoroplethMapColors.level4,
    },
    {
      label:
        bounds.level5.lower.text === bounds.level5.upper.text
          ? bounds.level5.upper.text
          : `${bounds.level5.lower.text}-${bounds.level5.upper.text}`,
