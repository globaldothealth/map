...
>("country/fetchCountriesData", async (_, { rejectWithValue, getState }) => {
  ...
  try {
    const { url } = await getUrl({
      path: s3Path,
      options: {
        bucket: { bucketName: "aggregated-map-data", region: "eu-central-1" },
      },
    });
    ...
    const confirmedCaseCount = countriesData
      .filter((entry) => entry["status"] === "confirmed")
      .reduce((total, entry) => total + entry.caseCount, 0);
    const probableCaseCount = countriesData
      .filter((entry) => entry["status"] === "probable")
      .reduce((total, entry) => total + entry.caseCount, 0);
