...
export const usePathingLayer = (
  ...
  const buildSignificantEventsGeoJSON = (
    events: typeof significantEventsData,
  ) => {
    ...
    for (const e of events) {
      if (!grouped[e.location]) {
        grouped[e.location] = {
          ...
          entries: [],
          popupAnchor:
            (e as typeof e & { popupAnchor?: string }).popupAnchor || "bottom",
        };
      }
    ...
    return {
      ...
      features: Object.entries(grouped).map(([location, group]) => ({
        ...
        geometry: {
          type: "Point" as const,
          coordinates: [
            locationNameToLongLat[location].long,
            locationNameToLongLat[location].lat,
          ],
        },
      })),
    };
  ...
  useEffect(() => {
    ...
    if (!map.getSource("ship")) {
      const filteredShipPath = shipPath.filter(
        (sp) => sp.dateStart <= dateUpTo,
      );
      map.addSource("ship", {
        type: "geojson",
        data: {
          ...
          features: [
            {
              ...
              geometry: {
                ...
                coordinates: filteredShipPath.map((sp) => [
                  locationNameToLongLat[sp.location].long,
                  locationNameToLongLat[sp.location].lat,
                ]),
              },
            },
          ],
      ...
      map.addSource("transfers", {
        ...
        data: {
          ...
          features: filteredTransfers
            .filter(
              (t) =>
                locationNameToLongLat[t.from] && locationNameToLongLat[t.to],
            )
            .map((t) => ({
              type: "Feature" as const,
              properties: {
              ...
              geometry: {
                ...
                coordinates: [
                  [
                    locationNameToLongLat[t.from].long,
                    locationNameToLongLat[t.from].lat,
                  ],
                  [
                    locationNameToLongLat[t.to].long,
                    locationNameToLongLat[t.to].lat,
                  ],
                ],
              },
            })),
    ...
    if (!map.getSource("significant-events")) {
      const filteredEvents = significantEventsData.filter(
        (e) => locationNameToLongLat[e.location] && e.dateStart <= dateUpTo,
      );
      ...
      map.on("mouseenter", "significant-events-circle", (e) => {
        ...
        let html =
          '<div style="min-width:300px;max-height:600px;overflow-y:auto;font-size:14px;white-space:pre-wrap;padding:10px;">';
    ...
    const setupLayer = async () => {
      try {
        ...
        const joinedFeatures: Feature[] = boundaries.features.map((feature) => {
          ...
          const matchedData = data.find(
            (d: CountryData | StateData | RegionalData) => {
              if (adminLevel === 0) {
                // For countries, match by ISO code if available
                return (
                  d.countryCode?.toUpperCase() === shapeGroup?.toUpperCase()
                );
              }
              // For sub-national, match by name within the same country
              return (
                d.name?.toLowerCase() === shapeName?.toLowerCase() &&
                d.countryCode?.toUpperCase() === shapeGroup?.toUpperCase()
              );
            },
          );
          ...
          const newProps: Record<string, any> = {
            ...props,
            areaName: matchedData ? matchedData.name : shapeName,
  ...
  useEffect(() => {
    ...
    if (map.getSource("transfers")) {
      ...
      (map.getSource("transfers") as any).setData({
        ...
        features: filteredTransfers
          .filter(
            (t) => locationNameToLongLat[t.from] && locationNameToLongLat[t.to],
          )
          .map((t) => ({
            type: "Feature" as const,
            properties: {
    ...
    if (map.getSource("significant-events")) {
      ...
      (map.getSource("significant-events") as any).setData(
        buildSignificantEventsGeoJSON(filteredEvents),
      );
    }
  }, [dateUpTo, map]);
  ...
  useEffect(() => {
    ...
    const filter =
      dataType === DataType.Confirmed
        ...
        : undefined;
    [`admin${adminLevel}Join`, `admin${adminLevel}JoinBorder`].forEach(
      (layerId) => {
  ...
  useEffect(() => {
    ...
    ["transfers-line", "transfers-arrow", "transfers-label"].forEach(
      (layerId) => {
        if (map.getLayer(layerId)) {
          map.setLayoutProperty(layerId, "visibility", visibility);
        }
  ...
  useEffect(() => {
    ...
    [
      "significant-events-circle",
      "significant-events-marker-text",
      "significant-events-label",
    ].forEach((layerId) => {
      if (map.getLayer(layerId)) {
        map.setLayoutProperty(layerId, "visibility", visibility);
      }
  ...
  useEffect(() => {
    ...
    const foundArea = data.find(
      (rd: CountryData | StateData | RegionalData) => rd.areaId === areaId,
    );
    ...
    if (foundArea) {
      ...
      const lastUploadDate = convertStringDateToDate(foundAreaData.lastUpdated);
