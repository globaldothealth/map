...
const SideBar = () => {
  ...
  const lastUpdateDate = useAppSelector(selectCountryLastUpdateDate);
  ...
  const mapDataToAutocomplete = (
    ...
    const mappedData = administrativeAreaData.map((administrativeAreaEntry) => {
      return {
        name: administrativeAreaEntry.name || "",
        areaId: administrativeAreaEntry.areaId,
        countryCode:
          whereAlpha3(administrativeAreaEntry.countryCode)?.alpha2 || "",
      };
    });
      ...
      .filter(
        (administrativeAreaEntry) =>
          ...
          (dataType == DataType.Confirmed &&
            administrativeAreaEntry.status === "confirmed"),
      )
      ...
      <>
        {administrativeAreaData.map((administrativeAreaEntry) => {
          ...
          return (
            <LocationListItem
              key={areaId}
              onClick={() => handleAreaClick(name, areaId, countryCode)}
              data-cy="listed-country"
              isActive={isActive}
            >
              <>
                {!countryCode.startsWith("Other") ? (
                  <FlagIcon
                    loading="lazy"
                ...
                <CountryLabel isActive={isActive} variant="body2">
                  {name}{" "}
                  {status === "probable" && (
                    <span style={{ color: "#bac0be" }}>{status}</span>
                  )}
                </CountryLabel>
              </>
              <CountryCaseCount isActive={isActive} variant="body2">
        ...
        return (
          <>
            <div>
              ...
              <span className="reported-cases-label">total cases</span>
            </div>
            <div>
              <span className="reported-cases-label">
      ...
      <SearchBar className="searchbar">
        <Autocomplete
          ...
          renderOption={(props, option) => (
            <Box component="li" className="autocompleteBox" {...props}>
